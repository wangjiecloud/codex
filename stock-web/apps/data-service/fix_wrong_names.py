...
"""
import sqlite3, json, baostock as bs
...
for code, correct_name, market, industry_ids in fixes:
    ...
    # 查当前状态
    old = cur.execute("SELECT name, industry_ids FROM stock_meta WHERE code=?", (code,)).fetchone()
    print(f"  当前DB: name={old[0] if old else 'NOT IN DB'}, ids={old[1] if old else '?'}")
    
    if old:
        # 合并已有的 industry_ids
        old_ids = json.loads(old[1]) if old[1] else []
        new_ids = list(set(old_ids + industry_ids))
        cur.execute("UPDATE stock_meta SET name=?, industry_ids=?, market=?, updated_at=? WHERE code=?",
    ...
    old_count = cur.execute("SELECT COUNT(*) FROM stock_kline WHERE code=?", (code,)).fetchone()[0]
    ...
    rs = bs.query_history_k_data_plus(
        bs_code, "date,code,open,high,low,close,volume,amount,turn,pctChg",
    ...
    if rows:
        for r in rows:
            cur.execute("""
                ...
                VALUES (?,?,?,?,?,?,?,?,?,?,?)
            """, (code, "daily", r[0],
                  round(safe_float(r[2]),4), round(safe_float(r[3]),4),
                  round(safe_float(r[4]),4), round(safe_float(r[5]),4),
                  int(safe_float(r[6])), safe_float(r[7]),
                  round(safe_float(r[9]),4), NOW))
    ...
    if profit_row:
        raw = json.dumps({"profit": profit_row, "growth": growth_row, "balance": balance_row}, ensure_ascii=False)
        cur.execute("""
            ...
            VALUES (?,?,?,?,?,?,?,?,?,?)
        """, (code, profit_row[0], safe_float(profit_row[2]),

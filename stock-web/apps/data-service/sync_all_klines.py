...
from industry_stocks import INDUSTRY_A_SHARES
...
ok, fail = 0, 0
total = len(INDUSTRY_A_SHARES)
for i, code in enumerate(INDUSTRY_A_SHARES, 1):
    try:
        ...
        while rs.error_code == "0" and rs.next():
            ...
            stmt = sqlite_insert(StockKline).values(
                code=code, period="daily", trade_date=r[0],
                open=safe_float(r[2]), high=safe_float(r[3]),
                low=safe_float(r[4]),  close=safe_float(r[5]),

...
class StockF10FinancialStatement(Base):
    ...
    updated_at = Column(DateTime, default=datetime.utcnow)
    __table_args__ = (
        UniqueConstraint("code", "statement_type", "tab_label", "report_date"),
    )
...
class MarketDailyFundFlow(Base):
    """大盘资金流向历史（来源：东方财富 AKShare stock_market_fund_flow，每日一行）"""

    __tablename__ = "market_daily_fund_flow"
    id = Column(Integer, primary_key=True, autoincrement=True)
    trade_date = Column(String(10), unique=True, index=True)  # YYYY-MM-DD
    # 指数收盘价与涨跌幅
    sh_close = Column(Float, default=0.0)  # 上证收盘价
    sh_change_pct = Column(Float, default=0.0)  # 上证涨跌幅（%）
    sz_close = Column(Float, default=0.0)  # 深证收盘价
    sz_change_pct = Column(Float, default=0.0)  # 深证涨跌幅（%）
    # 主力资金（净流入=大单+超大单）
    main_net = Column(Float, default=0.0)  # 主力净流入额（元）
    main_net_pct = Column(Float, default=0.0)  # 主力净流入占比（%）
    # 超大单（机构）
    super_net = Column(Float, default=0.0)
    super_net_pct = Column(Float, default=0.0)
    # 大单（游资/大户）
    big_net = Column(Float, default=0.0)
    big_net_pct = Column(Float, default=0.0)
    # 中单
    mid_net = Column(Float, default=0.0)
    mid_net_pct = Column(Float, default=0.0)
    # 小单（散户）
    small_net = Column(Float, default=0.0)
    small_net_pct = Column(Float, default=0.0)
    updated_at = Column(DateTime, default=datetime.utcnow)
...
class MarketFundFlowSnapshot(Base):
    ...
    )  # north_bound | main | institution | hot_money | retail
...
def init_db():
    ...
    with engine.connect() as conn:
        ...
        ffs_cols = {
            row[1]
            for row in conn.execute(text("PRAGMA table_info(fund_flow_snapshot)"))
        }
        if "period" not in ffs_cols:
            # SQLite 不支持修改唯一约束，需重建表
            conn.execute(text("DROP INDEX IF EXISTS ix_fund_flow_snapshot_trade_date"))
            conn.execute(
                text("ALTER TABLE fund_flow_snapshot RENAME TO fund_flow_snapshot_old")
            )
            conn.execute(
                text("""
        ...
        for col_def in [
            ...
            if col_def[0] not in sf_cols:
                conn.execute(
                    text(
                        f"ALTER TABLE stock_fundamental ADD COLUMN {col_def[0]} {col_def[1]}"
                    )
                )
        conn.commit()

...
def main():
    ...
    for code, (name, market, industry_ids) in NEW_COMPANIES.items():
        ...
        existing = cur.execute("SELECT code FROM stock_meta WHERE code=?", (code,)).fetchone()
        if not existing:
            cur.execute("""
            ...
            # 更新 industry_ids
            old = json.loads(cur.execute("SELECT industry_ids FROM stock_meta WHERE code=?", (code,)).fetchone()[0] or "[]")
            new_ids = list(set(old + industry_ids))
            cur.execute("UPDATE stock_meta SET industry_ids=? WHERE code=?", (json.dumps(new_ids), code))
            print(f"  [meta] 更新 industry_ids: {new_ids}")

        # 2. 写入 stock_quote 默认值（如果不存在）
        qrow = cur.execute("SELECT code FROM stock_quote WHERE code=?", (code,)).fetchone()
        if not qrow:
            cur.execute("""
        ...
        kline_count = cur.execute("SELECT COUNT(*) FROM stock_kline WHERE code=?", (code,)).fetchone()[0]
        if kline_count < 100:
        ...
        # 4. 同步 fundamental
        fund_row = cur.execute("SELECT code FROM stock_fundamental WHERE code=?", (code,)).fetchone()
        if not fund_row:

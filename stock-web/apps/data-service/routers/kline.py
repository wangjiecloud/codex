...
def _fetch_and_cache_klines(code: str, period: str, count: int) -> list:
    ...
    try:
        ...
        start = (date.today() - timedelta(days=max(count * 2, 400))).strftime("%Y-%m-%d")
        end = date.today().strftime("%Y-%m-%d")
        fields = "date,code,open,high,low,close,volume,amount,turn,pctChg"
        rs = bsapi.query_history_k_data_plus(
            bs_code, fields, start_date=start, end_date=end, frequency=bs_period, adjustflag="2"
        )
        bars = []
        while rs.error_code == "0" and rs.next():
            r = rs.get_row_data()
            stmt = sqlite_insert(StockKline).values(
                code=code, period=period, trade_date=r[0],
                open=_safe_float(r[2]), high=_safe_float(r[3]),
                low=_safe_float(r[4]), close=_safe_float(r[5]),
                volume=int(_safe_float(r[6])), turnover=_safe_float(r[7]),
                change_pct=_safe_float(r[9]), updated_at=datetime.utcnow(),
            )
            stmt = stmt.on_conflict_do_update(
                ...
                set_={
                    ...
                    "volume": stmt.excluded.volume, "turnover": stmt.excluded.turnover,
            ...
            db.execute(stmt)
            bars.append({
                "time": r[0], "open": _safe_float(r[2]),
                "high": _safe_float(r[3]), "low": _safe_float(r[4]),
                "close": _safe_float(r[5]), "volume": int(_safe_float(r[6])),

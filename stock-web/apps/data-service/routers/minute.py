...
数据来源：新浪财经 stock_zh_a_minute（1分钟K线，覆盖最近约8个交易日）

GET  /api/minute/{code}?date=YYYY-MM-DD
  - 先查数据库缓存
  - 数据库无数据时：最近8天用新浪1分钟拉取
...
def _read_from_db(code, trade_date):
    ...
    try:
        rows = (
            ...
            .filter(
                StockMinuteKline.code == code, StockMinuteKline.trade_date == trade_date
            )
...
async def get_minute_kline(
    ...
    if not bars:
        raise HTTPException(
            status_code=404, detail=f"{trade_date} 无分时数据（非交易日或停牌）"
        )
    return {
        "code": code,
        "date": trade_date,
...
def _get_industry_codes() -> List[str]:
    ...
    try:
        ...
        return [
            ...
            if r[0]
            and len(r[0]) == 6
            and (r[0].startswith("0") or r[0].startswith("3") or r[0].startswith("6"))
        ]
    except Exception:
        return []
    finally:
...
def _run_industry_sync(trade_date: str):
    ...
    sched_log(
        "info",
        f"[分时同步] 开始，共 {len(codes)} 只产业链A股，交易日 {trade_date}",
        source="minute_sync",
    )
    for code in codes:
    ...
    sched_log(
        "success",
        f"[分时同步] 完成：新增 {ok} 只，缓存 {cached} 只，失败 {error} 只",
        source="minute_sync",
    )

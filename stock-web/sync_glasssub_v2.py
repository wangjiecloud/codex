    ...
    for row in rows:
        ...
        try:
            ...
            """, (code, 'daily', trade_date,
                  safe_float(open_) or 0, safe_float(high) or 0,
                  safe_float(low) or 0, safe_float(close) or 0,
                  safe_float(volume) or 0, safe_float(turnover) or 0,
...
def sync_fundamental(conn, code, market):
    ...
    if not profit_row:
        ...
        return 0
    raw = json.dumps({"profit": profit_row, "growth": growth_row, "balance": balance_row})
    ...
    revenue_yoy = safe_float(growth_row[6]) if growth_row and len(growth_row) > 6 else None
    net_profit_yoy = safe_float(growth_row[4]) if growth_row and len(growth_row) > 4 else None
    debt_ratio = safe_float(balance_row[6]) if balance_row and len(balance_row) > 6 else None
    cursor = conn.cursor()
    cursor.execute("""
        ...
        VALUES (?,?,?,?,?,?,?,?,?)
    """, (code, report_date, eps, revenue_yoy, net_profit, net_profit_yoy, gross_margin, debt_ratio, raw))
    conn.commit()
...
def main():
    ...
    for c in NEW_COMPANIES:
        ...
        if not cursor.fetchone():
            cursor.execute("INSERT INTO stock_meta (code, name, market, industry_ids) VALUES (?,?,?,?)",
